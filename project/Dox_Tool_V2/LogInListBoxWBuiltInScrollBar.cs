		...
		protected override void OnPaint(PaintEventArgs e)
		{
			...
			checked
			{
				...
				for (int i = num4; i <= num5; i++)
				{
					...
					int num6 = i * ItemHeight + 1 - num + (int)Math.Round((double)ItemHeight / 2.0 - 8.0);
					if (_SelectedItems.Contains(logInListBoxItem))
					{
						graphics2.FillRectangle(new SolidBrush(_SelectedItemColour), new Rectangle(0, i * ItemHeight + 1 - num, base.Width - 19, ItemHeight - 1));
					}
					else
					{
						graphics2.FillRectangle(new SolidBrush(_NonSelectedItemColour), new Rectangle(0, i * ItemHeight + 1 - num, base.Width - 19, ItemHeight - 1));
					}
					graphics2.DrawLine(new Pen(_BorderColour), 0, i * ItemHeight + 1 - num + ItemHeight - 1, base.Width - 18, i * ItemHeight + 1 - num + ItemHeight - 1);
					graphics2.DrawString(logInListBoxItem.Text, new Font("Segoe UI", 8f), new SolidBrush(_TextColour), 9f, num6);
					graphics2.ResetClip();
				}
				graphics2.DrawRectangle(new Pen(Color.FromArgb(35, 35, 35), 2f), 1, 1, base.Width - 2, base.Height - 2);
				graphics2.DrawLine(new Pen(_BorderColour, 2f), VerticalScrollbar.Location.X - 1, 0, VerticalScrollbar.Location.X - 1, base.Height);
